...
def main():
    ...
    if run_mini_batch_size_lr_experiments:
        ...
        if config["data"] == "all" or config["data"] == "circle":
        ...
        if config["data"] == "all" or config["data"] == "mnist":
            ...
            mnist_experiment_log.to_csv(results_dir + f"mnist_mini_batch_size_lr_experiment_log_{config['optimizer']}_{config['loss_function']}.csv",
                                        sep=",", header=True, index=False, encoding="utf-8")
            mnist_training_logs.to_csv(results_dir + f"mnist_mini_batch_size_lr_training_logs_{config['optimizer']}_{config['loss_function']}.csv",
                                       sep=",", header=True, index=False, encoding="utf-8")

        if config["data"] == "all" or config["data"] == "fashion_mnist":
            ...
            fashion_mnist_experiment_log.to_csv(results_dir + f"fashion_mnist_mini_batch_size_lr_experiment_log_{config['optimizer']}_{config['loss_function']}.csv",
                                                sep=",", header=True, index=False, encoding="utf-8")
            fashion_mnist_training_logs.to_csv(results_dir + f"fashion_mnist_mini_batch_size_lr_training_logs_{config['optimizer']}_{config['loss_function']}.csv",
                                               sep=",", header=True, index=False, encoding="utf-8")
        print("Done!")
